...
try:
  with open(f"build/mem_writes", 'r') as input_file:
  ...
  print(f"Error: The file 'build/mem_writes' was not found.", file=sys.stderr)
  sys.exit(1)
...
try:
  with open(f"build/spike.sym", 'r') as input_file:
    for line in input_file:
  ...
  print(f"Error: The file 'build/spike.sym' was not found.", file=sys.stderr)
  sys.exit(1)
...
# Store Data in intex hex format in build/test_data
OUTPUT_STRING = ""
CNT = 0
...
try:
  with open(f"build/test_data", 'w') as output_file:
    output_file.write(OUTPUT_STRING)
except FileNotFoundError:
  print(f"Error: The file 'build/test_data' was not found.", file=sys.stderr)
  sys.exit(1)
